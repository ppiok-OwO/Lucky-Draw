...
let blessingExplain = (player) => {
  if (player.blessing === 'Spike Defender') {
    console.log(
      colors.green2(
        '| 가시 수호자는 기본 가시데미지를 20 얻습니다. 방어도를 얻을 때 현재 가시 데미지 수치의 절반만큼을 방어도로 획득합니다. 가시 데미지를 가진 상태에서 공격을 받을 시, 공격자에게 가시 데미지를 줄 수 있습니다. 이때, 공격에 의해 방어도가 파괴되면 가시데미지가 절반으로 줄어듭니다..\n',
      ),
    );
  } else if (player.blessing === 'Berserker') {
    console.log(
